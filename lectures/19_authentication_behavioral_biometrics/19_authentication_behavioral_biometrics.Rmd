...
subtitle: "19: Authentication Using Behavioral Biometrics"
...
Starting with the following code, let us create a model to predict whether a user is `s002` or not. 
...
other_subjects = other_subjects.drop(columns = ['sessionIndex', 'rep']).sample(400, random_state = 2025)

sub2_df = pd.concat( [sub2, other_subjects], axis=0)

sub2_df['target'] = sub2_df['subject'].apply(lambda x: 1 if x == 's002' else 0)

setup(
  data = sub2_df, target = 'target', session_id = 2025, 
